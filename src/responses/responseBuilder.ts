import { Tips, createBabyTips } from "./guides.js";

type Topic = keyof Tips;
...
const detectTopics = (text: string): Topic[] => {
...
const chooseTopics = (found: Topic[], tips: Tips): Topic[] => {
  if (found.length > 0) return found;
  return ["feeding", "sleep", "risk"].filter((topic) => topic in tips) as Topic[];
};

export const buildBabyResponse = (text: string, ageMonths: number): string => {
