...
const normalizeAge = (raw: number): number => Math.max(0, Math.floor(raw ?? 0));

const feedingGuidance = (ageMonths: number): string => {
  if (ageMonths <= 1)
...
const sleepGuidance = (ageMonths: number): string => {
  if (ageMonths <= 1)
...
const foodGuidance = (ageMonths: number): string => {
  if (ageMonths < 6)
...
const riskGuidance = (ageMonths: number): string => {
  const base = "如果出现呼吸困难、持续高烧、高频呕吐、嗜睡或拒食超过 6 次，请优先就医。";
  if (ageMonths <= 6) return `${base} 新生儿期更需关注吃奶、尿布、体温浮动。`;
  if (ageMonths <= 24)
    return `${base} 学步/爆发期可能因感染、脱水或脱臼加重，别忽略运动与喝水。`;
  return `${base} 孩子在断奶阶段也可能因困倦或肠胃症状加剧。`;
};

const cryGuidance = (ageMonths: number): string => {
  if (ageMonths <= 6)
...
export const createBabyTips = (ageMonths: number): Tips => {
